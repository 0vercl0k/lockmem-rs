...
pub(crate) fn turn_on_vt() -> Result<()> {
    ...
    if !unsafe { GetConsoleMode(h, &raw mut mode) }.as_bool() {
        return Err(format!("GetConsoleMode failed w/ {}", windows_core::Error::from_thread()).into());
    }

    mode |= ENABLE_VIRTUAL_TERMINAL_PROCESSING;
    if !unsafe { SetConsoleMode(h, mode) }.as_bool() {
        return Err(format!("SetConsoleMode failed w/ {}", windows_core::Error::from_thread()).into());
    }
